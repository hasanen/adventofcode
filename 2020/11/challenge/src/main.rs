...
fn main() {
    ...
    while true {
        ...
        if seats_take_on_last_round == seats_taken { break }
        seats_take_on_last_round = seats_taken;
    }

    println!("After {} rounds, {} seats have been taken", rounds, seats_taken(&map));
}

fn seats_taken(state: &Vec<Vec<char>>) -> usize {
    state.iter().map(|row| row.iter().filter(|c| c == &&SEAT_TAKEN).count() ).sum()
}
...
fn adjacent_seats_taken(row: usize, column: usize, state: &Vec<Vec<char>>) -> usize {
    let mut seats = 0;
    if row > 0 { //seats above
        if column > 0 && state[row - 1][column - 1] == SEAT_TAKEN { seats += 1 }
        if state[row - 1][column] == SEAT_TAKEN { seats += 1 }
        if column + 1 < state[row - 1].len() && state[row - 1][column + 1] == SEAT_TAKEN { seats += 1 }
    }

    if column > 0 && state[row][column - 1] == SEAT_TAKEN { seats += 1 }
    if column + 1 < state[row].len() && state[row][column + 1] == SEAT_TAKEN { seats += 1 }

    if row + 1 < state.len() { //seats below
        if column > 0 && state[row + 1][column - 1] == SEAT_TAKEN { seats += 1 }
        if state[row + 1][column] == SEAT_TAKEN { seats += 1 }
        if column + 1 < state[row + 1].len() && state[row + 1][column + 1] == SEAT_TAKEN { seats += 1 }
    }
